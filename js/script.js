// js/script.js (The Final, Corrected, Production-Ready Version)

// A SINGLE event listener that wraps ALL our code.
document.addEventListener('DOMContentLoaded', () => {
    ...
    // =======================================================
    // == SECTION 1: AUTOMATIC BACKGROUND IMAGE CHANGER     ==
    // =======================================================
    ...
    async function loadTestList() {
        const longLoadTimer = setTimeout(() => { if (testListContainer) testListContainer.innerHTML = '<p class="text-info">The server is waking up, this can take up to a minute. Please be patient...</p>'; }, 8000);
        try { const response = await fetch(`${liveServerUrl}/api/tests`); clearTimeout(longLoadTimer); if (!response.ok) throw new Error(`Server responded: ${response.status}`); const tests = await response.json(); if (tests.length === 0) { testListContainer.innerHTML = '<p>No tests added yet.</p>'; return; } testListContainer.innerHTML = ''; tests.forEach(test => { const testLink = document.createElement('a'); testLink.href = `mcq-test.html?test=${test.name}`; testLink.className = 'list-group-item list-group-item-action d-flex justify-content-between align-items-center'; testLink.innerHTML = `<strong>${test.name.replace(/-/g, ' ').toUpperCase()}</strong><span class="badge bg-primary rounded-pill">${test.questionCount} Questions</span>`; testListContainer.appendChild(testLink); }); } catch (error) { clearTimeout(longLoadTimer); console.error('Failed to load test list:', error); if (testListContainer) testListContainer.innerHTML = '<p class="text-danger">Could not load tests. Please refresh the page in a moment.</p>'; }
    }

    async function startTest(testName) {
        mainContentBox.innerHTML = `<h2>${testName.replace(/-/g, ' ').toUpperCase()}</h2><hr><div id="test-area"><p>Loading questions...</p></div>`; const testArea = document.getElementById('test-area'); const longLoadTimer = setTimeout(() => { if (testArea) testArea.innerHTML = '<p class="text-info">The server is waking up. Please be patient...</p>'; }, 8000); const url = `${liveServerUrl}/api/test/${testName}`; try { const response = await fetch(url); clearTimeout(longLoadTimer); if (!response.ok) throw new Error(`Server responded: ${response.status}`); const questions = await response.json(); displayQuestions(questions, testName); } catch (error) { clearTimeout(longLoadTimer); console.error("Could not fetch questions:", error); if(testArea) testArea.innerHTML = `<p class="text-danger">Failed to load test. Please go back and try again.</p>`; }
    }
    ...
    if (runButton) {
        ...
        editor.getWrapperElement().addEventListener('contextmenu', handleDisabledAction);
...
}); // The ONE AND ONLY closing tag for the DOMContentLoaded listener.