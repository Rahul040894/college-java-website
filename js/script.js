...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (runButton) {
        ...
        activateEditorProctoring(editor); // Activate editor proctoring
        const stdInput = document.getElementById('stdInput');
        const outputArea = document.getElementById('outputArea');
        runButton.addEventListener('click', async () => { /* ... existing compiler run logic ... */ });
    }

    // --- Coding Problems Logic (with Full Proctoring) ---
    const codingContainer = document.getElementById('coding-container');
    if (codingContainer) { loadProblemList(); }
    
    async function loadProblemList() {
        isSubmitting = false;
        deactivateProctoring(); // Ensure proctoring is off on the list page
