// js/script.js (The Final, Fully-Robust Production Version with All UX Fixes)
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- MCQ System Functions (with Cold Start UX) ---

    async function loadTestList() {
        const testListContainer = document.getElementById('test-list-container');
        // Start a timer to show a more patient message if loading takes time
        ...
        try {
            ...
            clearTimeout(longLoadTimer); // If we get a response, clear the timer
            ...
            clearTimeout(longLoadTimer); // Also clear the timer on an error
            console.error('Failed to load test list:', error);
            if (testListContainer) testListContainer.innerHTML = '<p class="text-danger">Could not load tests. The server might be busy. Please try refreshing the page in a moment.</p>';
        }
    }
    ...
    // --- Online Compiler Logic (already robust) ---
