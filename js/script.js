// js/script.js (The Truly Final Production Version with Hardened Proctoring)
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // This function now takes the CodeMirror editor instance as an argument
    function activateEditorProctoring(editorInstance) {
        // Defense Layer 1: CodeMirror's native events
        ...
        // Defense Layer 2: Low-level DOM events on the editor's wrapper
        ...
        // Defense Layer 3: The definitive 'beforeChange' check to cancel pastes
        editorInstance.on('beforeChange', (instance, changeObj) => {
            if (changeObj.origin === 'paste') {
                changeObj.cancel(); // Physically stops the text from being inserted
    ...
    function deactivatePageProctoring() {
    ...
    if (runButton) {
        ...
        activateEditorProctoring(editor); // Activate the new, hardened editor proctoring
    ...
    async function loadProblemList() {
        isSubmitting = false; deactivatePageProctoring();
    ...
    async function loadSingleProblem(problemId) {
        ...
        try {
            ...
            document.getElementById('backToListBtn').onclick = () => { deactivatePageProctoring(); loadProblemList(); };
            ...
            activateEditorProctoring(editor); // Activate the hardened editor proctoring
            const submissionCallback = () => { if(submitCodeBtn && !submitCodeBtn.disabled) submitCodeBtn.click(); };
            activatePageProctoring(submissionCallback); // Activate the page proctoring
