// js/script.js (The Truly Final, Complete, and Production-Ready Version)
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function checkExamStatus() {
        ...
        try {
            ...
            if (data.isLive) {
                examAlertBox.classList.remove('d-none');
            }
        } catch (error) {
            console.error("Could not check exam status:", error);
        }
    }
    if (examAlertBox) {
        checkExamStatus();
    ...
    const handleCheatingAttempt = (callback) => {
        if (!isSubmitting) {
            if (visibilityToast) visibilityToast.show();
            setTimeout(() => {
    ...
    function activatePageProctoring(submissionCallback) {
        const visibilityHandler = () => {
            if (document.hidden) handleCheatingAttempt(submissionCallback);
        };
    ...
    function deactivateProctoring() {
        if (window.proctoringListeners) {
            ...
            document.removeEventListener('blur', window.proctoringListeners.blurHandler);
    ...
    if (runButton) {
        ...
        runButton.addEventListener('click', async () => {
            ...
            const longLoadTimer = setTimeout(() => {
                outputArea.textContent = 'Server is waking up. Please be patient...';
            }, 8000);
            try {
                const response = await fetch(`${liveServerUrl}/api/compile`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ script: userCode, stdin: userInput })
                });
                ...
                if (result.error) {
                    ...
                    outputArea.textContent = "Execution finished, no output.";
                }
            } catch (error) {
                clearTimeout(longLoadTimer);
    ...
    async function loadProblemList() {
        ...
        codingContainer.innerHTML = `<h2>Coding Problems</h2><p>Select a problem. Each has a 30-minute time limit and is proctored.</p><hr><div id="problem-list"><p>Loading...</p></div>`;
        try {
        ...
        try {
            ...
            const submissionCallback = () => {
                if (submitCodeBtn && !submitCodeBtn.disabled) {
                    alert("Since you have navigated away from the test, your code is getting auto-submitted now! Attempt next question now!");
            ...
            runCodeBtn.addEventListener('click', async () => {
                ...
                try {
                    ...
                    if (result.output) {
                        ...
                        document.getElementById('outputArea').textContent = result.error;
                    }
                } catch (error) {
