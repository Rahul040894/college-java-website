...
document.addEventListener('DOMContentLoaded', () => {
    ...
    } else if (codingContainer) {
        // If the container exists but not the form, check if we're already logged in
        const loggedInUsn = localStorage.getItem('codingStudentId');
        if (loggedInUsn) {
            codingContainer.classList.remove('d-none');
            loadProblemList();
        } else {
            // This case should not be hit if starting from coding.html, but is a safeguard.
            const entryContainer = document.getElementById('coding-entry-container');
            if (entryContainer) entryContainer.classList.remove('d-none');
        }
    }
    
    // ... (rest of coding problems logic, especially submit which now pulls USN from localStorage)
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (codingEntryForm) {
        ...
        const loggedInUsn = localStorage.getItem('codingStudentId');
        if (loggedInUsn) { codingContainer.classList.remove('d-none'); loadProblemList(); } else { const entryContainer = document.getElementById('coding-entry-container'); if (entryContainer) entryContainer.classList.remove('d-none'); }
    }
        ...
        try {
            ...
            submitCodeBtn.addEventListener('click', async () => {
                ...
                try {
                    ...
                    if (!submitResponse.ok) { const errorData = await submitResponse.json(); throw new Error(errorData.message || 'Submission failed.'); }
