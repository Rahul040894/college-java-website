// js/script.js (The Truly Final, Fully-Stabilized Production Version)
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (runButton) { const editor = CodeMirror(document.getElementById('codeEditor'), { value: `public class MyClass {\n    public static void main(String args[]) {\n        System.out.println("Hello, World!");\n    }\n}`, mode: "text/x-java", theme: "dracula", lineNumbers: true, autoCloseBrackets: true }); editor.setSize(null, "500px"); activateEditorProctoring(editor); const languageSelector = document.getElementById('languageSelector'); if(languageSelector){ languageSelector.addEventListener('change', () => { const selectedLang = languageSelector.value; const newMode = selectedLang === 'java' ? 'text/x-java' : 'python'; editor.setOption('mode', newMode); editor.setValue( { java: `public class MyClass {\n    public static void main(String args[]) {\n        System.out.println("Hello, Java World!");\n    }\n}`, python3: `# Your Python code goes here!\nprint("Hello, Python World!")` }[selectedLang]); }); } const stdInput = document.getElementById('stdInput'); const outputArea = document.getElementById('outputArea'); runButton.addEventListener('click', async () => { const userCode = editor.getValue(); const userInput = stdInput.value; const selectedLang = languageSelector ? languageSelector.value : 'java'; runButton.disabled = true; runButton.innerHTML = `<span class="spinner-border spinner-border-sm"></span> Waking Server...`; outputArea.textContent = 'Connecting to the server...'; const longLoadTimer = setTimeout(() => { outputArea.textContent = 'Server is waking up. Please be patient...'; }, 8000); try { const response = await fetch(`${liveServerUrl}/api/compile`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ script: userCode, language: selectedLang, stdin: userInput }) }); clearTimeout(longLoadTimer); const result = await response.json(); if (result.error) { outputArea.textContent = result.error; } else if (result.output) { outputArea.textContent = result.output; } else { outputArea.textContent = "Execution finished, no output."; } } catch (error) { clearTimeout(longLoadTimer); outputArea.textContent = 'Could not connect. Please try again.'; } finally { runButton.disabled = false; runButton.innerHTML = `Run Code`; } }); }
    ...
    async function handleCodingAccess(e) { if (e) e.preventDefault(); const studentIdInput = document.getElementById('codingStudentId'); const studentId = studentIdInput.value.trim(); if (!studentId) { showError('coding-entry-error', 'Please enter your USN.'); return; } accessProblemsBtn.disabled = true; accessProblemsBtn.innerHTML = `<span class="spinner-border spinner-border-sm"></span> Verifying...`; try { const response = await fetch(`${liveServerUrl}/api/validate-usn`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ studentId }) }); const result = await response.json(); if (!result.valid) throw new Error(result.message || 'USN not authorized.'); localStorage.setItem('codingStudentId', result.usn); codingEntryContainer.classList.add('d-none'); codingContainer.classList.remove('d-none'); loadProblemList(); } catch (error) { showError('coding-entry-error', error.message); accessProblemsBtn.disabled = false; accessProblemsBtn.innerHTML = `Access Problems`; } }
        ...
        submitCodeBtn.addEventListener('click', async (event) => {
            ...
            try {
                ...
                if (!submitResponse.ok) {
                    ...
                    isSubmitting = false; // Reset flag before redirecting
                    loadProblemList();
                    return;
                }
