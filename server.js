// server.js (The Truly Final, Stabilized Production Version)
...
app.post('/api/exam/start', async (req, res) => {
    ...
    try {
        ...
        const isAllowed = await AllowedUsn.findOne({ usn: usnRegex });
        if (!isAllowed) {
            return res.status(403).json({ error: "This USN is not authorized." });
        }
        const existingResult = await TestResult.findOne({ studentId: usnRegex, testName });
        if (existingResult) {
            return res.status(403).json({ error: "This USN has already attempted this test." });
        }
        ...
        res.json({ questions: exam.questions.map(q => ({ id: q._id, question: q.question, options: q.options })), startTime: newResult.startTime });
    } catch (error) {
        if (error.code === 11000) {
            return res.status(403).json({ error: "This USN has already started the test." });
        }
...
app.post('/api/exam/submit', async (req, res) => {
    ...
    try {
        const usnRegex = new RegExp(`^${studentId}$`, 'i');
        const result = await TestResult.findOne({ studentId: usnRegex, testName });
