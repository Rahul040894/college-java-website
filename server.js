...
// 1. Model for public practice MCQ tests
...
// 2. Model for the fixed, timed exam questions
...
// 3. Model to store student results for the timed exam
const TestResultSchema = new mongoose.Schema({ studentName: String, studentId: String, testName: String, score: Number, total: Number, startTime: Date, finishTime: Date, status: String });
TestResultSchema.index({ studentId: 1, testName: 1 }, { unique: true });
const TestResult = mongoose.model('TestResult', TestResultSchema);

// 4. Model for individual coding problems
const CodingProblemSchema = new mongoose.Schema({ title: { type: String, required: true, unique: true }, description: String, exampleInput: String, exampleOutput: String, topic: String });
const CodingProblem = mongoose.model('CodingProblem', CodingProblemSchema);

// 5. Model for student code submissions
const CodeSubmissionSchema = new mongoose.Schema({ studentId: String, problemId: { type: mongoose.Schema.Types.ObjectId, ref: 'CodingProblem' }, submittedCode: String, submissionTime: { type: Date, default: Date.now } });
...
app.post('/api/exam/start', async (req, res) => { const { studentName, studentId, testName } = req.body; try { const exam = await Exam.findOne({ testName }); if (!exam) return res.status(404).json({ error: "Exam not available." }); const newResult = new TestResult({ studentName, studentId, testName, total: exam.questions.length }); await newResult.save(); res.json({ questions: exam.questions.map(q => ({ id: q._id, question: q.question, options: q.options })), startTime: newResult.startTime }); } catch (error) { if (error.code === 11000) return res.status(403).json({ error: "This USN has already started the test." }); res.status(500).json({ error: "Server error starting test." }); } });
...
// --- Endpoints for CODING PROBLEMS (RESTORED) ---
