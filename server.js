// server.js (The Truly Final Version with Single Submission Enforcement)
...
// This compound index is the database's guarantee against duplicates
CodeSubmissionSchema.index({ studentId: 1, problemId: 1 }, { unique: true });
const CodeSubmission = mongoose.model('CodeSubmission', CodeSubmissionSchema);


// === ALL API ENDPOINTS ===

// --- Feature Flag & USN Validation ---
...
// --- Coding Problems Endpoints (WITH UPDATED SUBMIT LOGIC) ---
...
app.post('/api/coding-problems/submit', async (req, res) => {
    ...
    try {
        ...
        const existingSubmission = await CodeSubmission.findOne({ studentId, problemId });
        ...
        // The unique index will act as a final backstop against race conditions
        if (error.code === 11000) {
            return res.status(403).json({ message: "You have already submitted a solution for this problem." });
        }
