...
app.get('/api/tests', async (req, res) => {
    try {
        ...
        res.json(testList);
    } catch (error) { res.status(500).json({ message: "Error fetching practice test list" }); }
});
...
app.get('/api/test/:testName', async (req, res) => {
    ...
    } catch (error) { res.status(500).json({ message: "Error fetching practice test" }); }
});
...
app.post('/api/submit/:testName', async (req, res) => {
    ...
    } catch (error) { res.status(500).json({ message: "Error submitting practice test" }); }
});
...
app.post('/api/exam/start', async (req, res) => {
    ...
    try {
        ...
        res.status(500).json({ error: "A server error occurred while starting the test. Please try again." });
    }
});
