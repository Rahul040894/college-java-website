// server.js (The Truly Final Production Version with API Key Pooling)
...
app.post('/api/validate-usn', async (req, res) => { /* ... existing code ... */ });
...
app.get('/api/coding-problems', async (req, res) => { /* ... existing code ... */ });
app.get('/api/coding-problems/:id', async (req, res) => { /* ... existing code ... */ });
app.post('/api/coding-problems/submit', async (req, res) => { /* ... existing code ... */ });

// --- UPDATED Online Compiler Endpoint (with Key Pooling) ---
app.post('/api/compile', async (req, res) => {
    const { script, language, stdin } = req.body;

    if (jDoodleCredentials.length === 0) {
        return res.status(500).json({ output: "Server is not configured for compilation. No API keys found." });
    }
    ...
    for (let i = 0; i < jDoodleCredentials.length; i++) {
        ...
        let langDetails = {};
        switch (language) {
            case 'python3': langDetails = { language: 'python3', versionIndex: '4' }; break;
            case 'java': default: langDetails = { language: 'java', versionIndex: '4' }; break;
        }

        const program = {
            script: script, stdin: stdin, language: langDetails.language, versionIndex: langDetails.versionIndex,
        ...
        try {
            ...
            // If the request is successful, but JDoodle says we are out of credits (statusCode 429)
            if (response.data.statusCode === 429) {
                console.warn(`⚠️ JDoodle Key ${keyIndexToTry + 1} has exhausted its credits. Trying next key...`);
                continue; // The loop will automatically continue to the next key
            }
            
            // SUCCESS! We found a working key.
            currentKeyIndex = keyIndexToTry; // Remember which key worked for the next request
    ...
    // If the loop finishes without a successful response from any key
    console.error("❌ All JDoodle API keys have been exhausted or are failing.");
    res.status(503).json({ output: "All available compiler resources are currently exhausted. Please try again later." });
});
...
// (Full code for other endpoints collapsed for brevity but should remain in your file)
app.post('/api/validate-usn', async (req, res) => { const { studentId } = req.body; if (!studentId) return res.status(400).json({ valid: false, message: 'USN is required.' }); try { const usnRegex = new RegExp(`^${studentId}$`, 'i'); const isAllowed = await AllowedUsn.findOne({ usn: usnRegex }); if (isAllowed) { res.json({ valid: true, usn: isAllowed.usn }); } else { res.json({ valid: false, message: 'This USN is not authorized.' }); } } catch (error) { res.status(500).json({ valid: false, message: 'Server error during validation.' }); } });
