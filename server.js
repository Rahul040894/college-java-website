// server.js (Final Version with Feature Flag)
...
// ========== NEW FEATURE FLAG ENDPOINT START ==========
// This endpoint checks the environment variable and reports the exam status.
app.get('/api/exam/status', (req, res) => {
    const isExamLive = process.env.EXAM_LIVE === 'true';
    res.json({ isLive: isExamLive });
});
// ========== NEW FEATURE FLAG ENDPOINT END ==========

// ... (All your other API endpoints for tests, compiler, etc., remain unchanged)

// === Server Start ===
app.listen(PORT, () => { console.log(`✅ Server is running on port ${PORT}`); });

// (Full code below for copy-paste to avoid errors)
...
app.get('/api/exam/status', (req, res) => { const isExamLive = process.env.EXAM_LIVE === 'true'; res.json({ isLive: isExamLive }); });
